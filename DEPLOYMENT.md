...
```bash
# 2. 一键上传至云端生产服务器
node upload-session.js ./nogi-browser-state.json https://<YOUR_APP_NAME>.fly.dev 管理员token
```
...
常规部署只需配置前文步骤中的 5 个 Secret。`ADMIN_TOKEN` 与 `CLIENT_TOKEN` 必须不同；旧 `ACCESS_TOKEN`/`API_KEY` 已不再用于鉴权。如需自定义其他变量，可在 `fly.toml` 的 `[env]` 区块按需覆盖：
