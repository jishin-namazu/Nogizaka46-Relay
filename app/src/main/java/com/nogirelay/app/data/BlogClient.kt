...
class BlogClient {
    ...
    fun fetchBlogPost(blogId: String, memberId: String? = null): BlogPost? {
        val targetId = blogId.trim()
        if (targetId.isEmpty()) return null

        runCatching {
            val page = requestPage(limit = 10, offset = 0)
            page.posts.firstOrNull { it.id == targetId && it.bodyHtml.isNotBlank() }
        }.getOrNull()?.let { return it }

        val cleanMemberId = memberId?.trim().orEmpty()
        if (cleanMemberId.isNotEmpty()) {
            runCatching {
                val page = requestPage(limit = 5, offset = 0, memberId = cleanMemberId)
                page.posts.firstOrNull { it.id == targetId && it.bodyHtml.isNotBlank() }
            }.getOrNull()?.let { return it }
        }

        return runCatching { fetchBlogPostFromWeb(targetId, cleanMemberId) }.getOrNull()
    }

    private fun fetchBlogPostFromWeb(blogId: String, memberId: String?): BlogPost? {
        val url = "https://www.nogizaka46.com/s/n46/diary/detail/$blogId"
        ...
        val bodyMatch = Regex("""<div\s+class="bd--edit"[^>]*>(.*?)</div>\s*<div\s+class="bd--foot">""", RegexOption.DOT_MATCHES_ALL).find(html)
            ?: Regex("""<div\s+class="bd--edit"[^>]*>(.*?)</div>""", RegexOption.DOT_MATCHES_ALL).find(html)
        val bodyHtml = bodyMatch?.groupValues?.get(1)?.trim().orEmpty()
    ...
    private fun requestPage(limit: Int, offset: Int, memberId: String? = null): BlogPage {
        ...
        if (!memberId.isNullOrBlank()) {
            builder.appendQueryParameter("ct", memberId)
        }
