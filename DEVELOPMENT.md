...
- **强制鉴权**：必须在 Header 中携带 `Authorization: Bearer <ADMIN_TOKEN>` 或客户端权限的 `CLIENT_TOKEN`，杜绝未经授权的公开盗链。
...
5. **空闲连接噪声抑制**：`pg` 连接池的空闲连接被上游/代理回收（`database.idle_client`）属于自动剔除死连接并重连的预期自愈行为。连接池默认 `min=0`、`maxLifetimeSeconds=1200`，在约 30 分钟的上游空闲超时前主动重建连接；残留的预期断连只在控制台按 `DB_IDLE_CLIENT_LOG_INTERVAL_MS` 汇总输出，不写入审计日志，真正的连接异常仍会持久化。
...
### 4.1 认证与通用规则
除 `/health` 外，接口须提供 Bearer Token。`ADMIN_TOKEN` 允许访问所有受保护接口；`CLIENT_TOKEN` 允许 `POST /v1/devices`，以及消息列表、详情、统计和媒体的 GET 请求（含 8081 媒体流）。旧 `ACCESS_TOKEN`/`API_KEY` 不再用于鉴权。两种 token 必须不同：
```http
Authorization: Bearer <ADMIN_TOKEN 或 CLIENT_TOKEN>
```
...
#### `POST /v1/admin/accounts/:id/renew`
立即触发指定账号的访问令牌主动续期，请求体为空。

#### `GET /v1/admin/push-logs`
分页查询推送记录。支持 `limit`（1–200）、`offset`、`status`（`success`/`failed`）、`deviceId` 与关键词 `q` 查询参数；返回推送记录、关联消息与设备信息及总数。
...
ADMIN_TOKEN=dev-admin-secret-token-123456
CLIENT_TOKEN=dev-client-secret-token-123456
...
- Android SDK 34/35，JDK 17 / 21 / 27，Gradle 9.8+。

#### 1. 注入 Firebase 配置文件
将 Firebase 项目的 `google-services.json` 放置于 `app/` 目录下（或者直接设置环境变量 `GOOGLE_SERVICES_JSON`，构建时会自动生成）。

#### 2. 本地个性化配置（支持环境变量或 local.properties）

**方式 A：使用本地环境变量构建（推荐，免去维护本地配置文件）：**
- **Android SDK**：设置系统的 `ANDROID_HOME` 或 `ANDROID_SDK_ROOT` 环境变量即可，无需在项目中配置 `sdk.dir`。
- **构建参数**（PowerShell 示例）：
  ```powershell
  $env:RELAY_BASE_URL = "https://your-relay-host.example.com"
  $env:RELAY_ACCESS_TOKEN = "your-secret-access-token"
  $env:RELAY_SIMPLE_UI = "false"  # 可选：设置为 true 启用精简 UI
  ```

**方式 B：传统 `local.properties` 文件方式：**
...
- **解决**：在本地运行 `npm run bootstrap:browser` 重新扫码/账密登录，再通过以下命令上传新会话：
  ```bash
  node upload-session.js ./nogi-browser-state.json https://YOUR_RELAY_SERVER YOUR_ADMIN_TOKEN
  ```
