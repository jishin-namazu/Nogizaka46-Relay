...
- **身份认证**：管理内容会在服务端验证 `ADMIN_TOKEN` 后显示。首次打开时输入令牌；凭据保存在当前浏览器中。

### 3.1 控制台核心功能模块
1. **多账号管理**：集中展示所有托管账号的状态、订阅群组列表、会话 Cookie 脱敏视图、访问令牌有效期进度条、续期状态与异常报错；支持拖拽上传会话文件、单个账号一键同步、主动续期、停用与删除。
2. **系统总览**：实时展示常驻内存占用 (RSS)、运行时间 (Uptime)、消息与多媒体归档统计、已注册设备平台分布及最近 24 小时错误计数。
3. **消息与媒体**：全量消息流视图，支持按账号筛选、按消息类型（文字/语音/图片/视频）筛选、关键词搜索、分页与跳转指定页、音频在线播放，图片与语音来电背景图支持页内放大预览。
4. **设备与推送**：在线查看已注册 FCM 设备列表与最后活跃时间，提供推送调试台（可直接发送测试普通通知与全屏拟真来电）。
5. **推送管理**：分页浏览全部推送记录，按推送状态、目标设备与关键词筛选，查看推送详情并按设备撤回已送达的通知，支持跳转指定页。
6. **审计日志**：实时检索持久化错误日志（按级别、范围与关键词过滤）。
...
#### 主动续期指定账号的访问令牌
```bash
curl -X POST "$SERVER/v1/admin/accounts/acc_sub_01/renew" \
  -H "Authorization: Bearer $TOKEN"
```
...
#### 分页查询推送记录
支持 `limit`（1–200）、`offset`、`status`（`success`/`failed`）、`deviceId` 与关键词 `q` 参数：
```bash
curl -s -H "Authorization: Bearer $TOKEN" \
  "$SERVER/v1/admin/push-logs?limit=20&offset=0&status=failed" | jq .
```
