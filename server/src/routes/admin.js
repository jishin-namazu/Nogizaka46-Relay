 ...
 * 分页查询推送记录（push_logs 关联 messages/devices），供管理端查看与撤回
 */
router.get('/push-logs', async (req, res) => {
  const limit = Math.min(Math.max(Number.parseInt(req.query.limit == null ? '30' : req.query.limit, 10) || 30, 1), 200);
  const offset = Math.max(Number.parseInt(req.query.offset == null ? '0' : req.query.offset, 10) || 0, 0);
  const status = req.query.status ? String(req.query.status).trim() : null;
  const query = req.query.q ? String(req.query.q).trim() : null;
  const deviceId = req.query.deviceId ? Number.parseInt(req.query.deviceId, 10) : null;

  if (status && status !== 'success' && status !== 'failed') {
    return res.status(400).json({ success: false, error: 'status must be success or failed' });
  }
  if (req.query.deviceId && !Number.isInteger(deviceId)) {
    return res.status(400).json({ success: false, error: 'deviceId must be an integer' });
  }
  if (query && query.length > 200) {
    return res.status(400).json({ success: false, error: 'q is too long' });
  }
  ...
  if (Number.isInteger(deviceId)) {
    ...
    conditions.push(`p.device_id = $${params.length}`);
  }
  if (query) {
    params.push(`%${query}%`);
    conditions.push(`(p.message_id ILIKE $${params.length} OR m.member_name ILIKE $${params.length} OR m.text ILIKE $${params.length} OR d.label ILIKE $${params.length})`);
  }
  ...
  try {
    ...
    const logs = await queryAll(
      `SELECT p.id, p.message_id, p.device_id, p.fcm_message_id, p.status, p.error_message, p.created_at,
              d.label AS device_label, d.platform AS device_platform,
              m.member_name, m.member_id, m.type, m.text, m.sent_at, m.media_url, m.thumbnail_url
       FROM push_logs p
       LEFT JOIN messages m ON m.id = p.message_id
       LEFT JOIN devices d ON d.id = p.device_id
       ${whereClause}
    ...
    res.json({ success: true, data: { total: totalCount, limit, offset, logs } });
  } catch (error) {
    await recordError('server.admin.push_logs', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
 ...
 * 立即触发指定账号的访问令牌主动续期（由 Monitor 进程执行真正的刷新）
 */
router.post('/accounts/:id/renew', async (req, res) => {
  try {
    const account = await getAccount(req.params.id);
    if (!account) {
      return res.status(404).json({ success: false, error: '账号不存在' });
    ...
    await touchAccountSignal('renew', req.params.id);
    res.json({
      success: true,
      message: '主动续期请求已下发',
    ...
    await recordError('server.admin.renew_account', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
