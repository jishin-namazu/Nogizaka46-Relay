...
import { rememberTransientPush, getLastTransientPush, resolveRevokeMessageId } from '../services/push-revoke.js';
...
router.post('/test-message', async (req, res) => {
  try {
    ...
    if (result.success) rememberTransientPush(testMessage.id, targetDeviceId);
...
router.post('/revoke', async (req, res) => {
  try {
    ...
    if (!message_id) {
      lastLog = await db.queryOne(
        'SELECT message_id, created_at FROM push_logs ORDER BY created_at DESC LIMIT 1'
      );
      if (!lastLog) {
        lastMessage = await db.queryOne(
          "SELECT id FROM messages WHERE id !~ '^test[-_]' ORDER BY sent_at DESC LIMIT 1"
        );
      }
    }

    // 测试推送不写 push_logs，必须结合本进程的临时推送记录，否则会撤回到一条无关消息
    const targetMessageId = resolveRevokeMessageId({
      explicitMessageId: message_id,
      transient: getLastTransientPush(),
      lastLog,
      lastMessage,
    });
    ...
    let tokens = [];
    if (targetDeviceId) {
      const singleToken = await deviceService.getTokenByDeviceId(targetDeviceId);
    ...
    if (tokens.length === 0) {
      return res.status(404).json({
        success: false,
        error: targetDeviceId ? 'Device not found' : 'No devices registered',
      });
    }
