...
class NogiBrowserMonitor {
  ...
  async pauseAuthentication(cause, context = {}) {
    ...
    if (this.onStateChange) {
      await this.onStateChange({
        ...
        tokenRefreshError: cause?.message || '鉴权暂停',
      }).catch(() => {});
    }
  ...
  async refreshAccessToken({ sessionActivation = false } = {}) {
    ...
    this.refreshPromise = (async () => {
      ...
      // 先落一个“续期中”状态，便于管理端实时展示续期进度
      if (this.onStateChange) {
        await this.onStateChange({
          tokenRefreshState: 'renewing',
  ...
  /**
   * 管理端主动续期：绕过暂停守卫，强制向官网刷新一次访问令牌。
   */
  async forceRenewToken() {
    try {
      await this.refreshAccessToken({ sessionActivation: true });
      if (this.authPaused) this.resumeAuthentication();
      return { ok: true };
    } catch (error) {
