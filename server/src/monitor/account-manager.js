        ...
        this.signalTimer = setTimeout(async () => {
          ...
          try {
            ...
            if (data?.timestamp && data.timestamp > this.lastSignalTimestamp) {
              ...
              if (data.action === 'renew' && data.accountId) {
                const entry = this.workers.get(data.accountId);
                if (entry) {
                  const result = await entry.worker.forceRenewToken();
                  if (result && result.ok) {
                    console.log('[AccountManager] 账号 ' + data.accountId + ' 已按管理端请求完成主动续期');
                  } else {
                    console.warn('[AccountManager] 账号 ' + data.accountId + ' 主动续期未成功: ' + ((result && result.error) || 'unknown'));
                  }
                  return;
                }
              }
